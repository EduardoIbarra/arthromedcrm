...
export default function NewLandingPage() {
  ...
  const [greetingEs, setGreetingEs] = useState('Hola {name}')
  const [greetingEn, setGreetingEn] = useState('Hello {name}')
  const [greetingZh, setGreetingZh] = useState('你好 {name}')
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [specRes, catRes, congRes] = await Promise.all([
          fetch('/api/catalog/specialties'),
          fetch('/api/catalogos'),
          fetch('/api/congresos')
        ])
        const [specJson, catJson, congJson] = await Promise.all([
          specRes.json(),
          catRes.json(),
          congRes.json()
        ])
        ...
        console.error('Error loading form choices:', err)
      }
    }
    loadData()
  }, [])
    ...
    <AppShell>
      <div className="max-w-4xl mx-auto space-y-6 animate-fade-in pb-12">
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-150 shadow-sm flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">{t('newLandingPage' as any) || 'Nueva Landing Page'}</h1>
            <p className="text-xs text-gray-500 mt-1">Crea una Landing Page pública y personalizable para doctores y congresos.</p>
          </div>
        </div>
