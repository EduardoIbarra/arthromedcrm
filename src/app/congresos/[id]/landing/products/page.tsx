...
import CongressProductsClient from './products-client'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    if (!congress) {
      return {
        title: 'Productos del Congreso no encontrado | Arthromed ERP'
      }
    }

    const title = `Productos Recomendados - ${congress.name} | Arthromed`
    const description = congress.description || 'Lista completa de productos recomendados para el congreso'
...
export default async function Page({ params }: Props) {
  ...
  return <CongressProductsClient initialCongress={initialCongress} />
}
