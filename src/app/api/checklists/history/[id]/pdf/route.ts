...
export async function GET(
  ...
  try {
    ...
    if (!entry) {
      return NextResponse.json({ error: 'Registro de checklist no encontrado' }, { status: 404 })
    }
    ...
    // 4. Generate QR Code pointing to the specific checklist details page
    const host = request.headers.get('host') || 'localhost:3000'
    const protocol = host.startsWith('localhost') ? 'http' : 'https'
    const validationUrl = `${protocol}://${host}/inventario/checklists?historyId=${id}`
    ...
    // Notes Box (if notes are present)
    ...
    return new NextResponse(blob, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="Reporte_Checklist_${id.replace('hlog_', '')}.pdf"`,
        'Cache-Control': 'no-store, max-age=0'
      }
    })
  } catch (err: any) {
    console.error('Error generating checklist PDF:', err)
    return NextResponse.json({ error: err.message || 'Error interno al generar PDF' }, { status: 500 })
  }
}
