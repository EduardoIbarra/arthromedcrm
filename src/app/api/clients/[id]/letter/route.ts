...
export async function POST(
  ...
  try {
    ...
    if (linesDb.length === 0) {
      return NextResponse.json({ error: 'No se encontraron las líneas de producto seleccionadas' }, { status: 400 })
    }
    ...
    if (!fs.existsSync(machotePath) || !fs.existsSync(firmaEricPath) || !fs.existsSync(firmaRicardoPath) || !fs.existsSync(logoPath)) {
      return NextResponse.json({ error: 'Falta alguna imagen de recurso (machote, firmas o logo en resources/img)' }, { status: 500 })
    }
    ...
    // Signatures
    // We place names at y = 90
    // And signatures images above them at y = 105
    const nameY = 85
