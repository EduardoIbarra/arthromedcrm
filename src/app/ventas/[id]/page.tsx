  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch clients
        const clientsRes = await fetch('/api/ventas/clients')
        let fetchedClients: { id: string; name: string }[] = []
        if (clientsRes.ok) {
        ...
        if (!res.ok) throw new Error('Failed to load sales record')
        ...
        setClientSearch(data.cliente_nombre)
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(json.error || 'Failed to update sales record')
      }
        ...
        <header className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-3">
              ...
              {t('editVenta' as any) || 'Editar Venta'}
            </h1>
          </div>
        </header>
